...
@section('content')
            ...
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
                ...
                <nav class="flex-shrink-0 mt-3 mt-sm-0 ms-sm-3" aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-alt">
                        ...
                        <li class="breadcrumb-item">
                            <a class="link-fx" href="{{ route('admin.courses.index') }}">Course</a>
                        </li>
            ...
            <div class="block-content">
                <form action="{{ route('admin.courses.update',$Course->id) }}" method="POST"
                        ...
                        <select class="form-control" id="category_id" name="category_id">
                            ...
                            @if (!empty($Categories))
                                @foreach ($Categories as $Category)
                                    <option
                                        value="{{ $Category->id }}" {{ $Course->category_id == $Category->id ? 'selected' : '' }} {{ old("category_id") == $Category->id ? 'selected' : '' }}>{{ $Category->name }}</option>
                                @endforeach
                            @endif
                        </select>
                        ...
                        <select class="form-control" id="teacher_id" name="teacher_id">
                            ...
                            @if (!empty($Teachers))
                                @foreach ($Teachers as $Teacher)
                                    <option
                                        value="{{ $Teacher->id }}" {{ $Course->teacher_id == $Teacher->id ? 'selected' : '' }} {{ old("teacher_id") == $Teacher->id ? 'selected' : '' }}>{{ $Teacher->name }}</option>
                                @endforeach
                            @endif
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        @endif
                        <br>
                        <img src="{{ asset($Course->intro_video_thumbnail) }}" alt="" width="100px">
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" class="form-control" id="after_discount_price" name="after_discount_price"
                               ...
                               value="{{ old("after_discount_price", $Course->after_discount_price) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-control" id="task_included" name="task_included">
                            <option
                                value="0" {{ $Course->task_included == "0" ? 'selected' : '' }} {{ old("task_included") == "0" ? 'selected' : '' }}>
                                No
                            </option>
                            <option
                                value="1" {{ $Course->task_included == "1" ? 'selected' : '' }} {{ old("task_included") == "1" ? 'selected' : '' }}>
                                Yes
                            </option>
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <select class="form-control" id="certificate" name="certificate">
                            <option
                                value="0" {{ $Course->certificate == "0" ? 'selected' : '' }} {{ old("certificate") == "0" ? 'selected' : '' }}>
                                No
                            </option>
                            <option
                                value="1" {{ $Course->certificate == "1" ? 'selected' : '' }} {{ old("certificate") == "1" ? 'selected' : '' }}>
                                Yes
                            </option>
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea class="form-control" rows="8" name="requirements"
                                  placeholder="Requirements">{{ old("requirements", $Course->requirements) }}</textarea>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea class="form-control" rows="8" name="what_will_you_learn"
                                  placeholder="What will you learn">{{ old("what_will_you_learn", $Course->what_will_you_learn) }}</textarea>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea class="form-control" rows="8" name="who_this_course_is_for"
                                  placeholder="Who this course is for">{{ old("who_this_course_is_for", $Course->who_this_course_is_for) }}</textarea>
    ...
    <div class="content">
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <div class="col-md-10">
                    <h3 class="block-title">Course Contents Table </h3>
                </div>
                <div class="col-md-2">
                    <a href="{{ route('admin.courses.contents.create',$Course->id) }}" class="btn btn-primary float-end">+ Add</a>
                </div>
            </div>
                ...
                <table class="table table-bordered table-striped table-vcenter js-dataTable-full">
                    ...
                    <tbody>
                        ...
                        @foreach ($Course->contents as $CourseContent)
                            <tr>
                                <td class="text-center fs-sm">{{ $CourseContent->id }}</td>
                                <td class="fw-semibold fs-sm">{{ $CourseContent->title }}</td>
                                <td class="fw-semibold fs-sm">{{ $CourseContent->lectures->count() }}</td>
                                <td class="fw-semibold fs-sm">{{ $CourseContent->lectures->sum('minutes') }}</td>
                                <td class="fs-sm">
                                    <form action="{{ route('admin.courses.contents.destroy', $CourseContent->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')

                                        <a href="{{ route('admin.courses.contents.lectures.index', $CourseContent->id) }}" class="btn btn-success"><i class="fa fa-eye"></i></a>
                                        <a href="{{ route('admin.courses.contents.edit', $CourseContent->id) }}" class="btn btn-primary"><i class="fa fa-pencil"></i></a>
